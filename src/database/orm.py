...
class FoodItemTable:
    ...
    def fetch_by_outlet(self, outlet: str) -> List[FoodItem]:
        cursor = self.conn.execute("SELECT * FROM food_item WHERE outlet = ?", (outlet,))
        return [FoodItem(**dict(row)) for row in cursor.fetchall()]

    def insert(self, name: str, outlet: str) -> None:
        self.conn.execute("INSERT INTO food_item (name, outlet) VALUES (?, ?)", (name, outlet))
        self.conn.commit()

    def update(self, id: int, name: str, outlet: str) -> None:
        self.conn.execute("UPDATE food_item SET name = ?, outlet = ? WHERE id = ?", (name, outlet, id))
        self.conn.commit()
...
class AnswerTable:
    ...
    def fetch(self, food_item_id: int, question_id: int) -> Optional[Answer]:
        cursor = self.conn.execute("SELECT * FROM answer WHERE food_item_id = ? AND question_id = ?", (food_item_id, question_id))
        row = cursor.fetchone()
        return Answer(**dict(row)) if row else None

    def fetch_by_question_id(self, question_id: int) -> List[Answer]:
        cursor = self.conn.execute("SELECT * FROM answer WHERE question_id = ?", (question_id,))
        return [Answer(**dict(row)) for row in cursor.fetchall()]

    def fetch_by_food_item_id(self, food_item_id: int) -> List[Answer]:
        cursor = self.conn.execute("SELECT * FROM answer WHERE food_item_id = ?", (food_item_id,))
        return [Answer(**dict(row)) for row in cursor.fetchall()]
    ...
    def insert(self, food_item_id: int, question_id: int, value: int) -> None:
        self.conn.execute("INSERT INTO answer (food_item_id, question_id, value) VALUES (?, ?, ?)", (food_item_id, question_id, value))
        self.conn.commit()

    def update_value(self, food_item_id: int, question_id: int, value: int) -> None:
        self.conn.execute("UPDATE answer SET value = ? WHERE food_item_id = ? AND question_id = ?", (value, food_item_id, question_id))
        self.conn.commit()

    def delete(self, food_item_id: int, question_id: int) -> None:
        self.conn.execute("DELETE FROM answer WHERE food_item_id = ? AND question_id = ?", (food_item_id, question_id))
        self.conn.commit()
...
class Database:
    def __init__(self, db_path: str):
        ...
        self.food_item = FoodItemTable(self.conn)
